...
class delmotor(QThread):
	...
	def __init__(self, axis, direct, dist, speed,typeval,pos):
		...
		self.currentpos = self.devs[2].get_pos()
	...
	def changevalue(self):
		if self.type == 'relative':
			...
			if self.direct == 'increase':
				...
				position1[self.axisnum] -= self.dist
			self.devs[2].goto_pos(position1,self.speed)
		...
		if self.type == 'getposition_m0':
			x = 'self.m0' in vars()
			if x == False:
				self.m0 = self.devs[2].get_pos()
				print('wtf')
			else:
				print('no update')

		if self.type == 'getposition_m1':
			self.m1 = self.devs[2].get_pos()

		if self.type == 'getposition_m2':
			self.m2 = self.devs[2].get_pos()
