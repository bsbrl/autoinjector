...
from motorcontrol.trajectorythread import trajectoryimplementor
...
class ControlWindow(QWidget):
    ...
    def GUIsetup(self):
        ...
        instruct2 = QLabel("  Step 2  ")
        instruct3 = QLabel("Pipette Angle")

        motorcalib_window_calibuttonx = QPushButton("Step 1.1", self)
        motorcalib_window_calibuttonx.clicked.connect(lambda: self.showdialog(xory='x'))
        motorcalib_window_calibutton2x = QPushButton("Step 1.2", self)
        motorcalib_window_calibutton2x.clicked.connect(self.motorcalib_step2)

        motorcalib_window_calibutton = QPushButton("Step 2.1", self)
        motorcalib_window_calibutton.clicked.connect(lambda: self.showdialog(xory='y'))
        motorcalib_window_calibutton2 = QPushButton("Step 2.2", self)
        ...
        motorcalib_window.addWidget(instruct1,1,0)
        motorcalib_window.addWidget(motorcalib_window_calibuttonx,1,1)
        motorcalib_window.addWidget(motorcalib_window_calibutton2x,1,2)
        motorcalib_window.addWidget(instruct2,2,0)
        ...
        motorcalib_window.addWidget(instruct3,3,0)
    ...
    def showdialog(self,xory):
        ...
        try:
            ...
            if xory == 'x': #this list will store x,y pixel values and is reset after each click of the first x calibration step (should only happen 1x per pipette)
                self.pixelsize = []

            msg.buttonClicked.connect(lambda: self.motorcalib_step1(xory=xory))
            retval = msg.exec_()
    ...
    def motorcalib_step1(self,xory):
        ...
        movey = delmotor(xory, 'increase', self.motorcalibdist, 1000,'relative',0)
        self.xory = xory
        movey.start()
    ...
    def calculatetheta(self,xcameraline,ycameraline):
        ...
        if self.xory == 'x':
            self.xmotortheta = np.arctan(float(ycameraline)/xcameraline)
            print(self.xmotortheta)
            self.xscale = self.motorcalibdist/ymotorline
            self.videoheightpixel = int(self.vidctrl.frame.shape[0])
            self.videowidthpixel = int(self.vidctrl.frame.shape[1])
            self.videoheightdist = (self.videoheightpixel)*(self.xscale/1000)
            self.videowidthdist = (self.videowidthpixel)*(self.xscale/1000)
            print("xsclae (nm per pixel" + str(self.xscale))
            pixelsizex = self.xscale/1000
            print("x pixel size = " +str(pixelsizex))
            self.pixelsize.append(pixelsizex)

        elif self.xory == 'y':
            self.ymotortheta = np.arctan(float(xcameraline)/ycameraline)
            print("ymotortheta = " + str(self.ymotortheta))
            ...
            pixelsizey = self.yscale/1000
            print("pixel size = " +str(pixelsizey))
            self.pixelsize.append(pixelsizey)
    ...
    def runalongedgetrajectory(self):
        ...
        xythetas = []
        xythetas.append(self.xmotortheta)
        xythetas.append(self.ymotortheta)
        print(xythetas)

        print("pixel size list = " + str(self.pixelsize))

        self.trajimplement = trajectoryimplementor(xythetas, self.thetaz, currentpos, self.pixelsize, approach, depthintissue, spacing, self.d.drawedgecoord1, self.ncell, self.speed)
